	...
	if (func == "reconcile" || func == "delay" || func == "invoice" || (func == "funding" && attr == null)) {
		...
		$.post("admin.php?"+decodeURIComponent(window.location.search.substring(1)),'',function (data){
			//$('#divLoading').hide();
		  	//console.log("a"+data);
			//eval(data);
			$('.table.table-bordered.dataTable tbody tr').detach();
			$('.table.table-bordered.dataTable tbody').append(data);
			$('.searchresults_count').html($('.table.table-bordered.dataTable tbody tr').length);
			new jBox('Confirm', {
				content: '您確定撥款?',
				confirmButton: '確定',
				cancelButton: '取消'
...
function Funding(FirmSno, Period, ExpectedRecordedDate) {
	console.log(FirmSno+'|'+Period);
	$.post("admin.php?func=setfunding",'FirmSno='+FirmSno+"&Period="+Period+"&ExpectedRecordedDate="+ExpectedRecordedDate,function (data){
			console.log(data);
			eval(data);
			$.post("admin.php?"+decodeURIComponent(window.location.search.substring(1)),'',function (data){
			//$('#divLoading').hide();
