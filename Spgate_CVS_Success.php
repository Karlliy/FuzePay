  ...
  //if(trim($obj['Status']) == "SUCCESS") $obj2 = json_decode($obj['Result'], true);
	...
	if ($obj->Status == "SUCCESS") {  
		...
		$_NotSendSuccess = $FirmRow["NotSendSuccess"];
	  	...
	  	CDbShell::query("SELECT FC.* FROM FirmCommission AS FC INNER JOIN PaymentFlow AS PF ON FC.PaymentFlowSno = PF.Sno WHERE FC.FirmSno = ".$FirmRow["Sno"]." AND PF.Type = '2' LIMIT 1");  
		...
		if ($obj->Result->PaymentType == "VACC") {
			...
			$StoreName = $Store. "[". $obj->Result->StoreID."]";
		...
		if (CDbShell::affected_rows() != 1 || CDbShell::GetErrorNo() != 0) {
			$fp = fopen('Log/Spgate_ATM/Spgate_CVS_OK_'.date("YmdHis").'.txt', 'a');
			...
			$fp = fopen('Log/Spgate_ATM/Spgate_CVS_SUCCESSLOG_'.date("YmdHis").'.txt', 'a');
