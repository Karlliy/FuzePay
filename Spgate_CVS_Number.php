<?php
	...
	//print_r($_POST);
	$fp = fopen('Log/Spgate_ATM/Spgate_CVSNumber_LOG_'.date("YmdHis").'.txt', 'a');
	fwrite($fp, " ---------------- 開始POST ---------------- \n\r");
...
if ($obj->Status == 'SUCCESS' ) {
    ...
    $ExpireDatetime = $obj->Result->ExpireDate . " ". $obj->Result->ExpireTime;
    $field = array('OrderID', 'VatmAccount', 'ExpireDatetime');
    $value = array($obj->Result->TradeNo, $obj->Result->CodeNo, $obj->Result->ExpireDate . " ". $obj->Result->ExpireTime);
    CDbShell::update('ledger', $field, $value, "CashFlowID = '".$obj->Result->MerchantOrderNo."'");

	$_Amt = $obj->Result->Amt;
	
    include 'StorePay.html';
    //echo "1|OK";
    ...
    if ($TakeNumberURL != '') {
		...
		$SendPOST['ExpireDatetime'] = $obj->Result->ExpireDate . " ". $obj->Result->ExpireTime;
		//$SendPOST['Store'] = "711";
        $SendPOST["CodeNo"] = $obj->Result->CodeNo;
