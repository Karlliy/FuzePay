...
$(function () {	
	...
	if (func == "reconcile" || func == "delay" || func == "invoice" || (func == "funding" && attr == null)) {
		...
		$.post("admin.php?"+decodeURIComponent(window.location.search.substring(1)),'',function (data){
			//$('#divLoading').hide();
		  	//console.log("a"+data);
			//eval(data);
			$('.table.table-striped tbody tr').detach();
			$('.table.table-striped tbody').append(data);
			$('.searchresults_count').html($('.table.table-striped tbody tr').length);
			new jBox('Confirm', {
				content: '您確定撥款?',
				confirmButton: '確定',
				cancelButton: '取消'
	...
	if (func == "Holiday") {
		$.post("admin.php?"+decodeURIComponent(window.location.search.substring(1)),'',function (data){
			...
			$('.searchresults_count').html($('.table.table-striped tbody tr').length);
			var sno;
			$('button[id="DelHolidayButton"]').on('click', function (Event) {
				sno = $(this).data('sno');
			});
	...
	$('button[id="AddHolidayButton"]').on('click', function (Event) {
