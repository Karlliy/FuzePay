<?php
	...
	/*$fp = fopen('Log/OKMart/LOG_'.date("YmdHi").'.txt', 'a');
	fwrite($fp, "UNENCODED_URL => ".$_SERVER["UNENCODED_URL"].PHP_EOL);	
	fwrite($fp, "Searche => ".$_Searched[COUNT($_Searched)-1].PHP_EOL);
	fclose($fp);*/
...
<DATA_5></DATA_5>
	...
	function Notify() {
		...
		//var_dump($_ReturnXML);
		@CDbShell::connect();
		CDbShell::query("SELECT F.*, L.PaymentName, L.MerTradeID, L.MerProductID, L.MerUserID, L.Total, L.NotifyURL FROM Ledger AS L INNER JOIN Firm AS F ON L.FirmSno = F.Sno WHERE L.VatmAccount = '".$_Code."'"); 
		...
		CDbShell::query("SELECT FC.* FROM FirmCommission AS FC INNER JOIN PaymentFlow AS PF ON FC.PaymentFlowSno = PF.Sno WHERE FC.FirmSno = ".$FirmRow["Sno"]." AND PF.Type = '5' AND FC.Enable = '1' LIMIT 1"); 
		if (CDbShell::num_rows() == 0) {
			CDbShell::query("SELECT FC.* FROM FirmCommission AS FC INNER JOIN PaymentFlow AS PF ON FC.PaymentFlowSno = PF.Sno WHERE FC.FirmSno = ".$FirmRow["Sno"]." AND PF.Type = '5' AND (FC.FeeRatio > 0 OR FC.FixedFee > 0) LIMIT 1");  
