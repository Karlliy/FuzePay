<?php
    ...
    if (0 == strcmp($_POST['Validate'], $Validate)) {
        @CDbShell::connect();
        CDbShell::query("SELECT F.*, L.PaymentName, L.MerTradeID, L.MerProductID, L.MerUserID, L.Total, L.Fee, L.NotifyURL FROM Ledger AS L INNER JOIN Firm AS F ON L.FirmSno = F.Sno WHERE L.CashFlowID = '".$_POST["MerTradeID"]."'"); 
        ...
        if ($_POST["RtnCode"] == "1") {
			...
			$value = array($FirmRow["PaymentName"], $Period, $ClosingDate, $ExpectedRecordedDate, intval($_POST["Amount"]), $_PaymentDate, $_PaymentDate, "0000", "交易成功", "0", $_POST["PayInfo"]);
