<?php
    ...
    $fp = fopen('../Log/KGI/Notify_LOG_'.date("YmdHis").'.txt', 'a');
	...
	$value = array($obj->SEQNO, "虛擬帳號-[凱基]", $Period, $ClosingDate, $ExpectedRecordedDate, intval($obj->AMT), $Fee, $_PaymentDate, $_PaymentDate, "0", "交易成功", "0", $obj->RACCNO);
	...
	if (CDbShell::affected_rows() == 1) {
		...
		if (('' != $SuccessURL || $NotifyURL != '') && $_NotSendSuccess == 0) {
			...
			if ($SuccessURL != '') {
				try {
					...
					$fp = fopen('../Log/KGI/Send_Notify_LOG_'.date('YmdHi').'.txt', 'a');
					...
					$fp = fopen('../KGI/CHB/Send_Notify_ErrLOG_'.date('YmdHi').'.txt', 'a');
			...
			$fp = fopen('../Log/KGI/Send_Notify_ErrLOG_'.date('YmdHi').'.txt', 'a');
			fwrite($fp, ' ---------------- Send_Notify_Err開始 ---------------- '.PHP_EOL);
			fwrite($fp, '$SuccessURL =>'.$SuccessURL.PHP_EOL);
		...
		$fp = fopen('../Log/KGI/Fail_LOG_'.date('YmdHi').'.txt', 'a');
