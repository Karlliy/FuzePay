<?php
    ...
    if (CDbShell::num_rows() == 0) {        #沒有存在再新增
        ...
        if (CDbShell::num_rows() != 0) {
            ...
            CDbShell::query("SELECT F.*, L.PaymentName, L.MerTradeID, L.MerProductID, L.MerUserID, L.Total, L.Fee FROM Ledger AS L INNER JOIN Firm AS F ON L.FirmSno = F.Sno WHERE L.FirmSno = '".$_FirmSno."'"); 
            $FirmRow = CDbShell::fetch_array();
            $SuccessURL = $FirmRow["SuccessURL"];
            $FailURL = $FirmRow["FailURL"];
        }else {     #非固定
            $_IsFixe = false;
            CDbShell::query("SELECT F.*, L.FirmSno, L.PaymentName, L.MerTradeID, L.MerProductID, L.MerUserID, L.Total, L.Fee FROM Ledger AS L INNER JOIN Firm AS F ON L.FirmSno = F.Sno WHERE L.VatmAccount = '".$_VirAccount."'"); 
            $FirmRow = CDbShell::fetch_array();
            $SuccessURL = $FirmRow["SuccessURL"];
            $FailURL = $FirmRow["FailURL"];
        ...
        CDbShell::query("SELECT FC.* FROM FirmCommission AS FC INNER JOIN PaymentFlow AS PF ON FC.PaymentFlowSno = PF.Sno WHERE FC.FirmSno = ".$_FirmSno." AND PF.Type = '1' LIMIT 1");  
        if (CDbShell::num_rows() == 0) {
            CDbShell::query("SELECT FC.* FROM FirmCommission AS FC INNER JOIN PaymentFlow AS PF ON FC.PaymentFlowSno = PF.Sno WHERE FC.FirmSno = ".$_FirmSno." AND PF.Type = '1' AND (FC.FeeRatio > 0 OR FC.FixedFee > 0) LIMIT 1");  
        }
