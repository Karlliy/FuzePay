<?php
    ...
    $fp = fopen('../Log/First/Notify_LOG_'.date("YmdHis").'.txt', 'a');
    ...
    $_VatmAccount           = trim(mb_substr($_POST["content"], 75, 14, "utf-8"));
    $_Amount                = intval(mb_substr($_POST["content"], 43, 11, "utf-8"));
    //$_Year          = trim(mb_substr($_POST["body"], 49, 3, "utf-8"));
    //$_DateTime      = trim(mb_substr($_POST["body"], 52, 10, "utf-8"));
    $_Results               = trim(mb_substr($_POST["content"], 72, 1, "utf-8"));
    $_TransactionNo         = trim(mb_substr($_POST["content"], 19, 7, "utf-8"));
    $_CardNumber            = trim(mb_substr($_POST["content"], 91, 19, "utf-8"));

    fwrite($fp, " ---------------- Log ----------------".PHP_EOL);
    fwrite($fp, "\$_VatmAccount =>".$_VatmAccount.PHP_EOL);	
    fwrite($fp, "\$_Amount =>".$_Amount.PHP_EOL);
    fwrite($fp, "\$_Results =>".$_Results.PHP_EOL);
    fwrite($fp, "\$_TransactionNo =>".$_TransactionNo.PHP_EOL);
    fwrite($fp, "\$_CardNumber =>".$_CardNumber.PHP_EOL);
    fclose($fp);

    if (preg_match("/^(41665)[0-9]{8,11}$/", $_VatmAccount)) {
        $_PaymentDate = date("Y-m-d H:i:s");

        @CDbShell::connect();
        CDbShell::query("SELECT L.Sno FROM Ledger AS L WHERE L.VatmAccount = '".$_VatmAccount."' AND L.OrderID = '".$_TransactionNo."'"); 
        if (CDbShell::num_rows() == 0) {
            CDbShell::query("SELECT F.*, L.PaymentName, L.MerTradeID, L.MerProductID, L.MerUserID, L.Total, L.Fee, L.NotifyURL FROM Ledger AS L INNER JOIN Firm AS F ON L.FirmSno = F.Sno WHERE L.VatmAccount = '".$_VatmAccount."' LIMIT 1"); 
            ...
            CDbShell::query("SELECT FC.* FROM FirmCommission AS FC INNER JOIN PaymentFlow AS PF ON FC.PaymentFlowSno = PF.Sno WHERE FC.FirmSno = ".$FirmRow["Sno"]." AND PF.Type = '1' AND FC.Enable = '1' AND PF.Kind = '固定虛擬帳號' AND Mode = '第一銀行' LIMIT 1");  
            if (CDbShell::num_rows() == 0) {
                CDbShell::query("SELECT FC.* FROM FirmCommission AS FC INNER JOIN PaymentFlow AS PF ON FC.PaymentFlowSno = PF.Sno WHERE FC.FirmSno = ".$FirmRow["Sno"]." AND PF.Type = '1' AND PF.Kind = '固定虛擬帳號' AND (FC.FeeRatio > 0 OR FC.FixedFee > 0) LIMIT 1");  
            }
            ...
            $field = array("FirmSno", "OrderID", "CashFlowID", "MerTradeID", "MerProductID", "MerUserID", "PaymentName", "PaymentCode", "Period", "ClosingDate", "ExpectedRecordedDate", "ClosingTotal", "Fee", "TransactionDate", "PaymentDate", "ResultCode", "ResultMesg", "State", "CardNumber", "VatmAccount");
            $value = array($FirmRow["Sno"], $_TransactionNo, $CashFlowID, $FirmRow['MerTradeID'], $FirmRow['MerProductID'], $FirmRow['MerUserID'], "固定虛擬帳號-第一銀行", "固定虛擬帳號-[第一銀行]", $Period, $ClosingDate, $ExpectedRecordedDate, $_Amount , $Fee, $_PaymentDate, $_PaymentDate, "0", "交易成功", "0", $_CardNumber, $_VatmAccount);
            //CDbShell::update("ledger", $field, $value, "VatmAccount = '".$_VatmAccount."' AND OrderID IS NULL");
                ...
                if (('' != $SuccessURL || $NotifyURL != '') && $_NotSendSuccess == 0) {
                    ...
                    if ($SuccessURL != '') {
                        try {
                            ...
                            $fp = fopen('../Log/First/Send_Notify_LOG_'.date('YmdHi').'.txt', 'a');
                            ...
                            $fp = fopen('../Log/First/Send_Notify_ErrLOG_'.date('YmdHi').'.txt', 'a');
