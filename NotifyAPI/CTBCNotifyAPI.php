...
$_VatmAccount   = CTBC_Code.trim(mb_substr($_POST["body"], 20, 14, "utf-8"));
$_Amount        = intval(mb_substr($_POST["body"], 34, 15, "utf-8")) / 100;
$_Year          = trim(mb_substr($_POST["body"], 49, 3, "utf-8"));
$_DateTime      = trim(mb_substr($_POST["body"], 52, 10, "utf-8"));
$_CardNumber    = trim(mb_substr($_POST["body"], 85, 19));

$_Date = $_Year + 1911;
$_PaymentDate = Date('Y-m-d H:i:s', strtotime($_Date.$_DateTime));

fwrite($fp, " ---------------- 開始Data ----------------".PHP_EOL);
fwrite($fp, "\$_VatmAccount =>".$_VatmAccount.PHP_EOL);	
fwrite($fp, "\$_Amount =>".$_Amount.PHP_EOL);	
fwrite($fp, "\$_Year =>".$_Year.PHP_EOL);	
fwrite($fp, "\$_DateTime  =>".$_DateTime .PHP_EOL);	
fwrite($fp, "\$_PaymentDate  =>".$_PaymentDate .PHP_EOL);	
...
$value = array($_POST["TransactionNo"], "虛擬帳號-[中信]", $Period, $ClosingDate, $ExpectedRecordedDate, $_Amount , $Fee, $_PaymentDate, $_PaymentDate, "0", "交易成功", "0", $_CardNumber);
...
if (CDbShell::affected_rows() == 1) {
    if (('' != $SuccessURL || $NotifyURL != '') && $_NotSendSuccess == 0) {
        ...
        if ($SuccessURL != '') {
            try {
                ...
                $fp = fopen('../Log/CTBC/Send_Notify_LOG_'.date('YmdHi').'.txt', 'a');
                ...
                $fp = fopen('../Log/CTBC/Send_Notify_ErrLOG_'.date('YmdHi').'.txt', 'a');
    ...
    $fp = fopen('../Log/CTBC/Fail_LOG_'.date('YmdHi').'.txt', 'a');
